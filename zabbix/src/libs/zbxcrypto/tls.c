...
#elif defined(HAVE_MBEDTLS21)
#  define ZBX_TLS_MIN_MAJOR_VER  MBEDTLS_SSL_MAJOR_VERSION_3
#  define ZBX_TLS_MIN_MINOR_VER  MBEDTLS_SSL_MINOR_VERSION_3
#  define ZBX_TLS_MAX_MAJOR_VER  MBEDTLS_SSL_MAJOR_VERSION_3
#  define ZBX_TLS_MAX_MINOR_VER  MBEDTLS_SSL_MINOR_VERSION_3
#  define ZBX_TLS_CIPHERSUITE_CERT 0     /* select only certificate ciphersuites */
#  define ZBX_TLS_CIPHERSUITE_PSK  1     /* select only pre-shared key ciphersuites */
#  define ZBX_TLS_CIPHERSUITE_ALL  2     /* select ciphersuites with certificate and PSK */
#endif
...
#elif defined(HAVE_MBEDTLS21)
ZBX_THREAD_LOCAL static mbedtls_x509_crt   *ca_cert    = NULL;
ZBX_THREAD_LOCAL static mbedtls_x509_crl   *crl      = NULL;
ZBX_THREAD_LOCAL static mbedtls_x509_crt   *my_cert    = NULL;
ZBX_THREAD_LOCAL static mbedtls_pk_context   *my_priv_key    = NULL;
ZBX_THREAD_LOCAL static mbedtls_entropy_context    *entropy    = NULL;
ZBX_THREAD_LOCAL static mbedtls_ctr_drbg_context *ctr_drbg   = NULL;
ZBX_THREAD_LOCAL static char     *err_msg    = NULL;
ZBX_THREAD_LOCAL static int      *ciphersuites_cert  = NULL;
ZBX_THREAD_LOCAL static int      *ciphersuites_psk = NULL;
ZBX_THREAD_LOCAL static int      *ciphersuites_all = NULL;
...
static void	zbx_make_personalization_string(unsigned char pers[64])
{
	...
	sha256_init(&ctx);
	sha256_starts(&ctx, 1);		/* use SHA-384 mode */

	thread_id = zbx_get_thread_id();
	sha256_update(&ctx, (const unsigned char *)&thread_id, sizeof(thread_id));

	zbx_timespec(&ts);

	if (0 != ts.ns)
		sha256_update(&ctx, (const unsigned char *)&ts.ns, sizeof(ts.ns));

	sha256_finish(&ctx, pers);
	sha256_free(&ctx);
}
#elif defined(HAVE_MBEDTLS21)
/**********************************************************************************
 ...
 *                                                                            *
 ******************************************************************************/
static void  polarssl_debug_cb(void *tls_ctx, int level, const char *filename, int lineno, const char *str)
{
 char  msg[1024];  /* Apparently 1024 bytes is the longest message which can come from PolarSSL 1.3.9 */
 ...
 zbx_strlcpy(msg, str, sizeof(msg));
 zbx_rtrim(msg, "\n");
 zabbix_log(LOG_LEVEL_TRACE, "PolarSSL debug: level=%d filename=%s lineno=%d \"%s\"", level, filename, lineno, msg);
}
#elif defined(HAVE_GNUTLS)
/******************************************************************************
 *                                                                            *
...
static void  zbx_tls_error_msg(int error_code, const char *msg, char **error)
{
 ...
 mbedtls_strerror(error_code, err, sizeof(err));
 *error = zbx_dsprintf(*error, "%s%s", msg, err);
}
#elif defined(HAVE_OPENSSL)
 ...
 * Purpose:                                                                   *
 *     compose a certificate validation error message by decoding PolarSSL    *
 *     mbedtls_ssl_get_verify_result() return value                           *
 *                                                                            *
 * Parameters:                                                                *
 *     flags   - [IN] result returned by PolarSSL                             *
...
static void  zbx_tls_cert_error_msg(unsigned int flags, char **error)
{
 const unsigned int  bits[] = { MBEDTLS_X509_BADCERT_EXPIRED, MBEDTLS_X509_BADCERT_REVOKED, MBEDTLS_X509_BADCERT_CN_MISMATCH,
       MBEDTLS_X509_BADCERT_NOT_TRUSTED, MBEDTLS_X509_BADCRL_NOT_TRUSTED,
       MBEDTLS_X509_BADCRL_EXPIRED, MBEDTLS_X509_BADCERT_MISSING, MBEDTLS_X509_BADCERT_SKIP_VERIFY, MBEDTLS_X509_BADCERT_OTHER,
       MBEDTLS_X509_BADCERT_FUTURE, MBEDTLS_X509_BADCRL_FUTURE,
       MBEDTLS_X509_BADCERT_KEY_USAGE, MBEDTLS_X509_BADCERT_EXT_KEY_USAGE, MBEDTLS_X509_BADCERT_NS_CERT_TYPE,
       0 };
 const char    *msgs[] = { "expired", "revoked", "Common Name mismatch",
       "self-signed or not signed by trusted CA", "CRL not signed by trusted CA",
       "CRL expired", "certificate missing", "verification skipped", "other reason",
...
static int zbx_is_ciphersuite_cert(const int *p)
{
 ...
 /* PolarSSL function mbedtls_ssl_ciphersuite_uses_psk() is not used here because it can be unavailable in some */
 /* installations. */
 if (NULL != (info = mbedtls_ssl_ciphersuite_from_id(*p)) && (MBEDTLS_KEY_EXCHANGE_ECDHE_RSA == info->key_exchange ||
     MBEDTLS_KEY_EXCHANGE_RSA == info->key_exchange) &&
     (MBEDTLS_CIPHER_AES_128_GCM == info->cipher || MBEDTLS_CIPHER_AES_128_CBC == info->cipher) &&
     0 == (info->flags & MBEDTLS_CIPHERSUITE_WEAK) &&
     (ZBX_TLS_MIN_MAJOR_VER > info->min_major_ver || (ZBX_TLS_MIN_MAJOR_VER == info->min_major_ver &&
     ZBX_TLS_MIN_MINOR_VER >= info->min_minor_ver)) &&
     (ZBX_TLS_MAX_MAJOR_VER < info->max_major_ver || (ZBX_TLS_MAX_MAJOR_VER == info->max_major_ver &&
     ZBX_TLS_MAX_MINOR_VER <= info->max_minor_ver)))
...
static int zbx_is_ciphersuite_psk(const int *p)
{
 ...
 if (NULL != (info = mbedtls_ssl_ciphersuite_from_id(*p)) && (MBEDTLS_KEY_EXCHANGE_ECDHE_PSK == info->key_exchange ||
     MBEDTLS_KEY_EXCHANGE_PSK == info->key_exchange) &&
     (MBEDTLS_CIPHER_AES_128_GCM == info->cipher || MBEDTLS_CIPHER_AES_128_CBC == info->cipher) &&
     0 == (info->flags & MBEDTLS_CIPHERSUITE_WEAK) &&
     (ZBX_TLS_MIN_MAJOR_VER > info->min_major_ver || (ZBX_TLS_MIN_MAJOR_VER == info->min_major_ver &&
     ZBX_TLS_MIN_MINOR_VER >= info->min_minor_ver)) &&
...
static int zbx_is_ciphersuite_all(const int *p)
{
 ...
 if (NULL != (info = mbedtls_ssl_ciphersuite_from_id(*p)) && (MBEDTLS_KEY_EXCHANGE_ECDHE_RSA == info->key_exchange ||
     MBEDTLS_KEY_EXCHANGE_RSA == info->key_exchange ||
     MBEDTLS_KEY_EXCHANGE_ECDHE_PSK == info->key_exchange ||
     MBEDTLS_KEY_EXCHANGE_PSK == info->key_exchange) &&
     (MBEDTLS_CIPHER_AES_128_GCM == info->cipher || MBEDTLS_CIPHER_AES_128_CBC == info->cipher) &&
     0 == (info->flags & MBEDTLS_CIPHERSUITE_WEAK) &&
     (ZBX_TLS_MIN_MAJOR_VER > info->min_major_ver || (ZBX_TLS_MIN_MAJOR_VER == info->min_major_ver &&
...
static unsigned int  zbx_ciphersuites(int type, int **suites)
{
 ...
 supported_suites = mbedtls_ssl_list_ciphersuites();
 ...
 *                                                                            *
 ******************************************************************************/
static int zbx_psk_cb(void *par, mbedtls_ssl_context *tls_ctx, const unsigned char *psk_identity,
   size_t psk_identity_len)
{
 const char  *__function_name = "zbx_psk_cb";
 unsigned char *psk;
 ...
 if (0 < psk_len)
 {
   ...
   if (0 == (res = mbedtls_ssl_set_hs_psk(tls_ctx, psk, psk_len)))
     return 0;
...
static void  zbx_log_ciphersuites(const char *title1, const char *title2, const int *cipher_ids)
{
 if (SUCCEED == zabbix_check_log_level(LOG_LEVEL_DEBUG))
 {
   ...
   for (p = cipher_ids; 0 != *p; p++)
     zbx_snprintf_alloc(&msg, &msg_alloc, &msg_offset, " %s", mbedtls_ssl_get_ciphersuite_name(*p));
...
#if defined(HAVE_POLARSSL) || defined(HAVE_MBEDTLS21) || defined(HAVE_GNUTLS)
/******************************************************************************
 *                                                                            *
 ...
 * Purpose:                                                                   *
 *     Print distinguished name (i.e. issuer, subject) into buffer. Intended  *
 *     to use as an alternative to PolarSSL mbedtls_x509_dn_gets() to meet    *
 *     application needs.                                                     *
 *                                                                            *
 * Parameters:                                                                *
 *     dn    - [IN] pointer to distinguished name                             *
 ...
 *                                                                            *
 * Comments:                                                                  *
 *     This function is derived from PolarSSL mbedtls_x509_dn_gets() and      *
 *     heavily modified to print RDNs in reverse order, to print UTF-8        *
 *     characters and non-printable characters in a different way than        *
 *     original mbedtls_x509_dn_gets() does and to return error messages.     *
 *     Multi-valued RDNs are not supported currently.                         *
 *                                                                            *
 ******************************************************************************/
static int zbx_x509_dn_gets(const mbedtls_x509_name *dn, char *buf, size_t size, char **error)
{
 const mbedtls_x509_name *node, *stop_node = NULL;
 const char  *short_name = NULL;
 char    *p, *p_end;
 ...
 while (1)
 {
   ...
   if (NULL != node->oid.p)
   {
     ...
     if (0 == mbedtls_oid_get_attr_short_name(&node->oid, &short_name))
     {
       if (p_end - 1 == p)
         goto small_buf;
     ...
     else  /* unknown OID name, write in numerical form */
     {
       ...
       if (0 < (res = mbedtls_oid_get_numeric_string(p, (size_t)(p_end - p), &node->oid)))
         p += (size_t)res;
       else
         goto small_buf;
     }
...
static void	zbx_log_peer_cert(const char *function_name, const zbx_tls_context_t *tls_ctx)
{
...
#elif defined(HAVE_MBEDTLS21)
 if (NULL == (cert = mbedtls_ssl_get_peer_cert(tls_ctx->ctx)))
 {
   zabbix_log(LOG_LEVEL_DEBUG, "%s() cannot obtain peer certificate", function_name);
 }
 else if (SUCCEED != zbx_x509_dn_gets(&cert->issuer, issuer, sizeof(issuer), &error))
 ...
 else if (0 > mbedtls_x509_serial_gets(serial, sizeof(serial), &cert->serial))
 {
   zabbix_log(LOG_LEVEL_DEBUG, "%s() cannot obtain peer certificate serial", function_name);
 }
 else
...
#elif defined(HAVE_MBEDTLS21)
 zabbix_log(LOG_LEVEL_DEBUG, "mbed TLS library (version %s)", MBEDTLS_VERSION_STRING_FULL);
#elif defined(HAVE_GNUTLS)
...
void	zbx_tls_init_child(void)
{
	...
	if (0 != (res = ctr_drbg_init(ctr_drbg, entropy_func, entropy, pers, 48)))
		/* PolarSSL sha256_finish() in SHA-384 mode returns an array "unsigned char output[64]" where result */
...
void zbx_tls_init_child(void)
{
 ...
 if (NULL != CONFIG_TLS_CA_FILE)
 {
   ca_cert = zbx_malloc(ca_cert, sizeof(mbedtls_x509_crt));
   mbedtls_x509_crt_init(ca_cert);

   if (0 != (res = mbedtls_x509_crt_parse_file(ca_cert, CONFIG_TLS_CA_FILE)))
   {
     if (0 > res)
     {
       zbx_tls_error_msg(res, "", &err_msg);
 ...
 if (NULL != CONFIG_TLS_CRL_FILE)
 {
   crl = zbx_malloc(crl, sizeof(mbedtls_x509_crl));
   mbedtls_x509_crl_init(crl);

   if (0 != (res = mbedtls_x509_crl_parse_file(crl, CONFIG_TLS_CRL_FILE)))
   {
     zbx_tls_error_msg(res, "", &err_msg);
     zabbix_log(LOG_LEVEL_CRIT, "cannot parse CRL file \"%s\": %s", CONFIG_TLS_CRL_FILE, err_msg);
     zbx_free(err_msg);
 ...
 if (NULL != CONFIG_TLS_CERT_FILE)
 {
   my_cert = zbx_malloc(my_cert, sizeof(mbedtls_x509_crt));
   mbedtls_x509_crt_init(my_cert);

   if (0 != (res = mbedtls_x509_crt_parse_file(my_cert, CONFIG_TLS_CERT_FILE)))
   {
     if (0 > res)
     {
       zbx_tls_error_msg(res, "", &err_msg);
 ...
 if (NULL != CONFIG_TLS_KEY_FILE)
 {
   my_priv_key = zbx_malloc(my_priv_key, sizeof(mbedtls_pk_context));
   mbedtls_pk_init(my_priv_key);

   /* The 3rd argument of mbedtls_pk_parse_keyfile() is password for decrypting the private key. */
   /* Currently the password is not used, it is empty. */
   if (0 != (res = mbedtls_pk_parse_keyfile(my_priv_key, CONFIG_TLS_KEY_FILE, "")))
   {
     zbx_tls_error_msg(res, "", &err_msg);
     zabbix_log(LOG_LEVEL_CRIT, "cannot parse the private key in file \"%s\": %s",
         CONFIG_TLS_KEY_FILE, err_msg);
   ...
   zabbix_log(LOG_LEVEL_DEBUG, "%s() loaded " ZBX_FS_SIZE_T "-bit %s private key from file \"%s\"",
       __function_name, (zbx_fs_size_t)mbedtls_pk_get_bitlen(my_priv_key), mbedtls_pk_get_name(my_priv_key),
       CONFIG_TLS_KEY_FILE);
 }
 ...
 entropy = zbx_malloc(entropy, sizeof(mbedtls_entropy_context));
 mbedtls_entropy_init(entropy);

 zbx_make_personalization_string(pers);

 ctr_drbg = zbx_malloc(ctr_drbg, sizeof(mbedtls_ctr_drbg_context));
...
int  zbx_tls_connect(zbx_socket_t *s, unsigned int tls_connect, char *tls_arg1, char *tls_arg2, char **error)
{
 ...
 s->tls_ctx = zbx_malloc(s->tls_ctx, sizeof(zbx_tls_context_t));
 s->tls_ctx->conf = zbx_malloc(NULL, sizeof(mbedtls_ssl_config));
 mbedtls_ssl_config_init(s->tls_ctx->conf);
 s->tls_ctx->ctx = zbx_malloc(NULL, sizeof(mbedtls_ssl_context));
 mbedtls_ssl_init(s->tls_ctx->ctx);

 if (0 != (res = mbedtls_ssl_config_defaults(s->tls_ctx->conf,
     MBEDTLS_SSL_IS_CLIENT, MBEDTLS_SSL_TRANSPORT_STREAM, MBEDTLS_SSL_PRESET_DEFAULT)))
 {
   zbx_tls_error_msg(res, "mbedtls_ssl_config_defaults(): ", error);
   goto out_pre_setup;
 }

 if (0 != (res = mbedtls_ssl_setup(s->tls_ctx->ctx, s->tls_ctx->conf)))
 {
   zbx_tls_error_msg(res, "mbedtls_ssl_setup(): ", error);
   goto out;
 }

 /* Set RNG callback where to get random numbers from */
 mbedtls_ssl_conf_rng(s->tls_ctx->conf, mbedtls_ctr_drbg_random, ctr_drbg);

 /* disable using of session tickets (by default it is enabled on client) */
 // mbedtls_ssl_conf_session_tickets(s->tls_ctx->conf, MBEDTLS_SSL_SESSION_TICKETS_DISABLED);

 if (SUCCEED == zabbix_check_log_level(LOG_LEVEL_TRACE))
 {
   /* Set our own debug callback function. The 3rd parameter of mbedtls_ssl_conf_dbg() we set to NULL. It will be */
   /* passed as the 1st parameter to our callback function and will be ignored there. */
   mbedtls_ssl_conf_dbg(s->tls_ctx->conf, polarssl_debug_cb, NULL);
 ...
 /* Set callback functions for receiving and sending data via socket. */
 /* Functions provided by PolarSSL work well so far, no need to invent our own. */
 mbedtls_ssl_set_bio(s->tls_ctx->ctx, &s->socket, mbedtls_net_send, mbedtls_net_recv, NULL);

 /* set protocol version to TLS 1.2 */
 mbedtls_ssl_conf_min_version(s->tls_ctx->conf, ZBX_TLS_MIN_MAJOR_VER, ZBX_TLS_MIN_MINOR_VER);
 mbedtls_ssl_conf_max_version(s->tls_ctx->conf, ZBX_TLS_MAX_MAJOR_VER, ZBX_TLS_MAX_MINOR_VER);

 if (ZBX_TCP_SEC_TLS_CERT == tls_connect)  /* use certificates */
 {
   mbedtls_ssl_conf_authmode(s->tls_ctx->conf, MBEDTLS_SSL_VERIFY_REQUIRED);
   mbedtls_ssl_conf_ciphersuites(s->tls_ctx->conf, ciphersuites_cert);

   /* set CA certificate and certificate revocation lists */
   mbedtls_ssl_conf_ca_chain(s->tls_ctx->conf, ca_cert, crl);

		if (0 != (res = mbedtls_ssl_conf_own_cert(s->tls_ctx->conf, my_cert, my_priv_key)))
		{
			zbx_tls_error_msg(res, "mbedtls_ssl_conf_own_cert(): ", error);
			goto out;
		}
	}
	else	/* use a pre-shared key */
	{
		mbedtls_ssl_conf_ciphersuites(s->tls_ctx->conf, ciphersuites_psk);
		...
		if (NULL == tls_arg2)	/* PSK is not set from DB */
		{
			...
			if (0 != (res = mbedtls_ssl_conf_psk(s->tls_ctx->conf, (const unsigned char *)my_psk, my_psk_len,
					(const unsigned char *)my_psk_identity, my_psk_identity_len)))
			{
				zbx_tls_error_msg(res, "mbedtls_ssl_conf_psk(): ", error);
				goto out;
			}
		}
		else
		{
			...
			if (0 != (res = mbedtls_ssl_conf_psk(s->tls_ctx->conf, (const unsigned char *)psk_buf, (size_t)psk_len,
					(const unsigned char *)tls_arg1, strlen(tls_arg1))))
			{
				zbx_tls_error_msg(res, "mbedtls_ssl_conf_psk(): ", error);
				goto out;
	...
	sec = zbx_time();
#endif
	while (0 != (res = mbedtls_ssl_handshake(s->tls_ctx->ctx)))
	{
#if defined(_WINDOWS)
		if (s->timeout < zbx_time() - sec)
			zbx_alarm_flag_set();
		...
		if (SUCCEED == zbx_alarm_timed_out())
		{
			*error = zbx_strdup(*error, "mbedtls_ssl_handshake() timed out");
			goto out;
		}

		if (MBEDTLS_ERR_SSL_WANT_READ != res && MBEDTLS_ERR_SSL_WANT_WRITE != res)
		{
			if (MBEDTLS_ERR_X509_CERT_VERIFY_FAILED == res)
			{
				/* Standard PolarSSL error message might not be very informative in this case. For */
				/* example, if certificate validity starts in future, PolarSSL 1.3.9 would produce a */
				/* message "X509 - Certificate verification failed, e.g. CRL, CA or signature check */
			...
			zbx_tls_error_msg(res, "mbedtls_ssl_handshake(): ", error);
			goto out;
		}
	}
	...
	else	/* pre-shared key */
	{
		if (SUCCEED == zabbix_check_log_level(LOG_LEVEL_DEBUG))
		{
			/* special print: s->tls_ctx->ctx->psk_identity is not '\0'-terminated */
			zabbix_log(LOG_LEVEL_DEBUG, "%s() PSK identity: \"%.*s\"", __function_name,
					(int)s->tls_ctx->conf->psk_identity_len, s->tls_ctx->conf->psk_identity);
		}
	}
	...
	{
		zabbix_log(LOG_LEVEL_DEBUG, "End of %s():SUCCEED (established %s %s)", __function_name,
				mbedtls_ssl_get_version(s->tls_ctx->ctx), mbedtls_ssl_get_ciphersuite(s->tls_ctx->ctx));
	}

	return SUCCEED;

out:	/* an error occurred */
	mbedtls_ssl_free(s->tls_ctx->ctx);
	zbx_free(s->tls_ctx->ctx);
out_pre_setup:
 mbedtls_ssl_config_free(s->tls_ctx->conf);
 zbx_free(s->tls_ctx->conf);
	zbx_free(s->tls_ctx);
out1:
	zabbix_log(LOG_LEVEL_DEBUG, "End of %s():%s error:'%s'", __function_name, zbx_result_string(ret),
			ZBX_NULL2EMPTY_STR(*error));
...
int  zbx_tls_accept(zbx_socket_t *s, unsigned int tls_accept, char **error)
{
 ...
 if (0 != (tls_accept & ZBX_TCP_SEC_TLS_CERT))
 {
   ...
   if (NULL != my_cert && 0 != (res = mbedtls_ssl_conf_own_cert(s->tls_ctx->conf, my_cert, my_priv_key)))
   {
     zbx_tls_error_msg(res, "mbedtls_ssl_conf_own_cert(): ", error);
     goto out;
   }
 ...
 if (0 != (tls_accept & ZBX_TCP_SEC_TLS_PSK))
 {
   /* for agentd the only possibility is a PSK from configuration file */
   if (0 != (program_type & ZBX_PROGRAM_TYPE_AGENTD) &&
       0 != (res = mbedtls_ssl_conf_psk(s->tls_ctx->conf, (const unsigned char *)my_psk, my_psk_len,
       (const unsigned char *)my_psk_identity, my_psk_identity_len)))
   {
     zbx_tls_error_msg(res, "mbedtls_ssl_conf_psk(): ", error);
     goto out;
   ...
   else if (0 != (program_type & (ZBX_PROGRAM_TYPE_PROXY | ZBX_PROGRAM_TYPE_SERVER)))
   {
     /* For server or proxy a PSK can come from configuration file or database. */
     /* Set up a callback function for finding the requested PSK. */
     mbedtls_ssl_conf_psk_cb(s->tls_ctx->conf, zbx_psk_cb, NULL);
   }
 }
 ...
 if ((ZBX_TCP_SEC_TLS_CERT | ZBX_TCP_SEC_TLS_PSK) == (tls_accept & (ZBX_TCP_SEC_TLS_CERT | ZBX_TCP_SEC_TLS_PSK)))
 {
   ...
   if (NULL != my_cert)
   {
     /* it can also be a case in agentd listener - when both certificate and PSK is allowed, e.g. */
     /* for switching of TLS connections from PSK to using a certificate */
     mbedtls_ssl_conf_ciphersuites(s->tls_ctx->conf, ciphersuites_all);
   }
   else
   {
     /* assume PSK, although it is not yet known will there be the right PSK available */
 ...
 info = mbedtls_ssl_ciphersuite_from_id(s->tls_ctx->ctx->session->ciphersuite);

 if (MBEDTLS_KEY_EXCHANGE_PSK == info->key_exchange ||
     MBEDTLS_KEY_EXCHANGE_DHE_PSK == info->key_exchange ||
     MBEDTLS_KEY_EXCHANGE_ECDHE_PSK == info->key_exchange ||
     MBEDTLS_KEY_EXCHANGE_RSA_PSK == info->key_exchange)
 {
   s->connection_type = ZBX_TCP_SEC_TLS_PSK;
...
#elif defined(HAVE_MBEDTLS21)
#  define ZBX_TLS_WRITE(ctx, buf, len) mbedtls_ssl_write(ctx, (const unsigned char *)(buf), len)
#  define ZBX_TLS_READ(ctx, buf, len)  mbedtls_ssl_read(ctx, (unsigned char *)(buf), len)
#  define ZBX_TLS_WRITE_FUNC_NAME    "mbedtls_ssl_write"
#  define ZBX_TLS_READ_FUNC_NAME   "mbedtls_ssl_read"
#  define ZBX_TLS_WANT_WRITE(res)    (MBEDTLS_ERR_SSL_WANT_WRITE == (res) ? SUCCEED : FAIL)
#  define ZBX_TLS_WANT_READ(res)   (MBEDTLS_ERR_SSL_WANT_READ == (res) ? SUCCEED : FAIL)
#elif defined(HAVE_GNUTLS)
...
ssize_t	zbx_tls_write(zbx_socket_t *s, const char *buf, size_t len, char **error)
{
 ...
 if (0 > res)
 {
   ...
   mbedtls_strerror(res, err, sizeof(err));
   *error = zbx_dsprintf(*error, "mbedtls_ssl_write() failed: %s", err);
...
ssize_t	zbx_tls_read(zbx_socket_t *s, char *buf, size_t len, char **error)
{
 ...
 if (0 > res)
 {
   ...
   mbedtls_strerror(res, err, sizeof(err));
   *error = zbx_dsprintf(*error, "mbedtls_ssl_read() failed: %s", err);
...
void	zbx_tls_close(zbx_socket_t *s)
{
 ...
 if (NULL != s->tls_ctx->ctx)
 {
   ...
   sec = zbx_time();
#endif
   while (0 > (res = mbedtls_ssl_close_notify(s->tls_ctx->ctx)))
   {
#if defined(_WINDOWS)
     if (s->timeout < zbx_time() - sec)
       zbx_alarm_flag_set();
     ...
     if (MBEDTLS_ERR_SSL_WANT_READ != res && MBEDTLS_ERR_SSL_WANT_WRITE != res)
     {
       zabbix_log(LOG_LEVEL_WARNING, "mbedtls_ssl_close_notify() with %s returned error code: %d",
           s->peer, res);
       break;
     }
   }
...
#elif defined(HAVE_MBEDTLS21)
 if (NULL == (peer_cert = mbedtls_ssl_get_peer_cert(s->tls_ctx->ctx)))
 {
   zabbix_log(LOG_LEVEL_WARNING, "no peer certificate, mbedtls_ssl_get_peer_cert() returned NULL");
   return FAIL;
 }
...
#elif defined(HAVE_MBEDTLS21)
 attr->psk_identity = (char *)s->tls_ctx->conf->psk_identity;
 attr->psk_identity_len = s->tls_ctx->conf->psk_identity_len;
#elif defined(HAVE_GNUTLS)
